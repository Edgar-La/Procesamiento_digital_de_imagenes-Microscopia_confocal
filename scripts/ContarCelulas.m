...
imwrite(Im1_C,'Imagen_Original_Grises.png');
cd ..; cd ..; cd scripts;
...
cd ..; cd Resultados; cd (Archivo);
imwrite(Celulas_Duda.*Im1_C,'Celulas_en_duda.png');
cd ..; cd ..; cd scripts;

hold off;
% Imagen de las celulas enumeradas
figure();imshow(Celulas_Divididas); title('Celulas enumeradas','Fontsize',15);
hold on;
for k=1:n4
    [r,c] = find(L4==k);
    rbar = mean(r);
...
cd ..; cd Resultados; cd (Archivo);
saveas(gcf,'Celulas_Segmentadas_Enumeradas.png');
imwrite(Celulas_Divididas,'Celulas_Segmentadas.png');
cd ..; cd ..; cd scripts;
...
ID_Canales = table(nombres, [1;2;3]);
ID_Canales.Properties.VariableNames = {'Marcaje', 'ID'};
...
writetable(ID_Canales, strcat('ID_Canales_',Archivo,'.csv'));
